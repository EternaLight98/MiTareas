...
function checkOverdueTasks() {
    try {
        ...
        if (updatedCount > 0) {
            saveAllTasks();
            showNotification(`${updatedCount} tarea(s) vencida(s) marcada(s) como finalizada(s)`, 'info');
        }
    } catch (error) {
...
function renderTaskItem(task) {
    try {
        const isTaskOverdue = task.status === 'pendiente' && task.dueDate && isOverdue(task.dueDate);
            ...
            <div class="task-item ${overdueClass}">
                ...
                <div class="task-meta">
                    <div class="task-due-date ${isTaskOverdue ? 'overdue' : ''}">
                        ${task.dueDate ? `📅 ${formatDisplayDate(task.dueDate)}` : 'Sin fecha límite'}
                    </div>
                    <div class="task-actions-group">
                        ...
                        ` : ''}
                        <button class="btn btn--sm btn--outline task-action-btn edit-task-btn" data-task-id="${task.id}">
                            ✏️ Editar
                        </button>
                        <button class="btn btn--sm btn--outline task-action-btn delete-task-btn" data-task-id="${task.id}">
                            🗑️ Eliminar
                        </button>
                    </div>
                </div>
...
function updateTaskStats() {
    try {
        ...
        const completedCount = tasks.filter(task => task.status === 'finalizada').length;
